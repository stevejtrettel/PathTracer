...
//    light.mat=makeLight(lightColor,lightIntensity);
    ...
    //----------- THE FLOOR -------------------------
        bottomWall.frame=makeFrameNormal(vec3(0,0.5,0), vec3(0,1,0));
    bottomWall.mat=makeDielectric(color,0.0,roughness);

    //----------- THE CEILING -------------------------
        topWall.frame=makeFrameNormal(vec3(0,8,0), vec3(0,-1,0));
    topWall.mat=makeLight(vec3(1,1,1),1.);


    //----------- THE FRONT -------------------------
        frontWall.frame=makeFrameNormal(vec3(0,0,-10), vec3(0,0,1));
