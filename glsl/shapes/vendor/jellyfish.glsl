//----------------------------------------------------------------------------
// JELLYFISH — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `jellyfish_sdf`, and the
...
float jellyfishDistance(vec3 p, float size){
    return jellyfish_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float jellyfishBound(vec3 p, float size){
