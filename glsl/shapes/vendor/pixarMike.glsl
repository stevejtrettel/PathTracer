//----------------------------------------------------------------------------
// PIXARMIKE — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `pixarMike_sdf`, and the
...
float pixarMikeDistance(vec3 p, float size){
    return pixarMike_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float pixarMikeBound(vec3 p, float size){
