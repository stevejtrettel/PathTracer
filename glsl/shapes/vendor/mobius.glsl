//----------------------------------------------------------------------------
// MOBIUS — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `mobius_sdf`, and the
...
float mobiusDistance(vec3 p, float size){
    return mobius_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float mobiusBound(vec3 p, float size){
