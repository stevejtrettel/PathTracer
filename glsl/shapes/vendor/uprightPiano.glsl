//----------------------------------------------------------------------------
// UPRIGHTPIANO — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `uprightPiano_sdf`, and the
...
float uprightPiano_sdf(vec3 p)
{
	p *= RotMat(vec3(0.,1.,0.), 3.14);
...
float uprightPianoDistance(vec3 p, float size){
    return uprightPiano_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float uprightPianoBound(vec3 p, float size){
