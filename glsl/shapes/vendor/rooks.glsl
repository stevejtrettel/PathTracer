//----------------------------------------------------------------------------
// ROOKS — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `rooks_sdf`, and the
...
#define isZero(x) (abs(x)<0.5)
#define STATEVEC (rez.xy-vec2(1.0))
...
float rooksDistance(vec3 p, float size){
    return rooks_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float rooksBound(vec3 p, float size){
