//----------------------------------------------------------------------------
// ELEPHANT — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `elephant_sdf`, and the
...
float elephantDistance(vec3 p, float size){
    return elephant_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float elephantBound(vec3 p, float size){
