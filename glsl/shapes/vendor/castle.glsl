//----------------------------------------------------------------------------
// CASTLE — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `castle_sdf`, and the
...
float castleDistance(vec3 p, float size){
    return castle_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float castleBound(vec3 p, float size){
