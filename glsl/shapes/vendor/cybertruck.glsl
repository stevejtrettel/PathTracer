//----------------------------------------------------------------------------
// CYBERTRUCK — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `cybertruck_sdf`, and the
...
float cybertruckDistance(vec3 p, float size){
    return cybertruck_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float cybertruckBound(vec3 p, float size){
