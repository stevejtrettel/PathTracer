//----------------------------------------------------------------------------
// BURGER — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `burger_sdf`, and the
...
float model(vec3 p) {
  ...
  if (d < .1) {
    ...
    float s = .2 * theSeed(p * 5.);
    return m - s;
  }
  return min(d, m);
}

float burger_sdf(vec3 p) {
...
float burgerDistance(vec3 p, float size){
    return burger_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float burgerBound(vec3 p, float size){
