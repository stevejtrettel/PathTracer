//----------------------------------------------------------------------------
// CHEESE — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `cheese_sdf`, and the
...
float cheeseDistance(vec3 p, float size){
    return cheese_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float cheeseBound(vec3 p, float size){
