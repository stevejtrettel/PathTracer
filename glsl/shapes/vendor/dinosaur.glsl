//----------------------------------------------------------------------------
// DINOSAUR — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `dinosaur_sdf`, and the
...
float dinosaurDistance(vec3 p, float size){
    return dinosaur_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float dinosaurBound(vec3 p, float size){
