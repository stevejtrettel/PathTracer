//----------------------------------------------------------------------------
// TEMPLE — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `temple_sdf`, and the
...
float templeDistance(vec3 p, float size){
    return temple_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float templeBound(vec3 p, float size){
