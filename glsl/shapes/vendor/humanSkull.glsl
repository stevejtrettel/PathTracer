//----------------------------------------------------------------------------
// HUMANSKULL — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `humanSkull_sdf`, and the
...
float humanSkullDistance(vec3 p, float size){
    return humanSkull_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float humanSkullBound(vec3 p, float size){
