//----------------------------------------------------------------------------
// TIEFIGHTER — a vendored model from NVIDIA's sdf-explorer corpus.
//
...
//
// PORTED, minimally: the corpus's bare `sdf(vec3)` became `tieFighter_sdf`, and the
...
float tieFighterDistance(vec3 p, float size){
    return tieFighter_sdf(p/size)*size;
}

// bounding sphere — see the header: loose on purpose, tighten by eye
float tieFighterBound(vec3 p, float size){
