...
vec4 enneper(vec4 x, vec4 y, vec4 z){
    vec4 x2 = tmul(x, x);
    vec4 y2 = tmul(y, y);
    vec4 z2 = tmul(z, z);
    vec4 z3 = tmul(z2, z);
    vec4 term1 = 0.5*(y2 - x2) + 2.0/9.0*z3 + 2.0/3.0*z;
    vec4 term2 = 0.25*(y2 - x2) - 0.25*tmul(z, x2 + y2 + 8.0/9.0*z2) + 2.0/9.0*z;
    return tmul(-term1, term1, term1) + 6.0*tmul(z, term2, term2);
}
