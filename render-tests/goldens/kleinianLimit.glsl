...
const vec3  KLEIN_P               = vec3(0.0, 0.0, -3.0);
const vec3  KLEIN_OFFSET          = vec3(0.0);
const vec2  KLEIN_BOX             = vec2(-0.8089, 0.68);
const vec3  KLEIN_INVERSIONCENTER = vec3(0.0, 1.0, 1.0);
const float KLEIN_INVERSIONRADIUS = 0.8;
const float KLEIN_SIZE            = 1.0;
const float KLEIN_FOLD            = 0.45;
const float KLEIN_FUDGE           = 0.275;

const vec3  LIGHT_P      = vec3(-7.0, 8.0, 2.0);
...
bool inside_klein(vec3 p){ return sdf_klein(p) < 0.0; }
...
bool insideOf(int id, vec3 p){
    if(id == ID_KLEIN){ return inside_klein(p); }
