...
const vec3  CORE_P      = vec3(2.0, 1.3, -1.2);
const float CORE_RADIUS = 0.8;

const vec3  SHELL_P      = vec3(2.0, 1.3, -1.2);
const float SHELL_RADIUS = 1.3;

const vec3  WAX_P      = vec3(-1.0, 1.3, -1.2);
const float WAX_RADIUS = 1.3;

const vec3  GLASS_P      = vec3(-4.0, 1.3, -1.2);
const float GLASS_RADIUS = 1.3;
...
float sdf_core(vec3 p){
    return sphereDistance(p - CORE_P, CORE_RADIUS);
}

float sdf_shell(vec3 p){
    return sphereDistance(p - SHELL_P, SHELL_RADIUS);
}

float sdf_wax(vec3 p){
    return sphereDistance(p - WAX_P, WAX_RADIUS);
}

float sdf_glass(vec3 p){
    return sphereDistance(p - GLASS_P, GLASS_RADIUS);
}
...
bool inside_core(vec3 p){ return sdf_core(p) < 0.0; }
bool inside_shell(vec3 p){ return sdf_shell(p) < 0.0 && sdf_core(p) >= 0.0; }
bool inside_wax(vec3 p){ return sdf_wax(p) < 0.0; }
bool inside_glass(vec3 p){ return sdf_glass(p) < 0.0; }
...
Medium medium_core(vec3 p){
    ...
    m.ior    = 1.4;
    m.absorb = absorbFor(coreTint, coreDepth);
    m.mfp    = coreDensity;
    m.blur   = 1.0;
    return m;
}
...
Medium medium_shell(vec3 p){
    Medium m = defaultMedium();
    m.ior    = 1.5;
    m.absorb = absorbFor(vec3(0.9, 0.93, 0.96), 3.0);
    return m;
}
Material material_shell(vec3 p, inout Vector n){
    Material m = defaultMaterial();      //glass
    m.interior = medium_shell(p);
    m.surf.transmit = 1.0;
    return m;
}

Medium medium_wax(vec3 p){
    Medium m = defaultMedium();
    m.ior    = 1.45;
    m.absorb = absorbFor(waxTint, waxDepth);
    m.mfp    = waxDensity;
    m.blur   = waxBlur;
    return m;
}
Material material_wax(vec3 p, inout Vector n){
    Material m = defaultMaterial();      //subsurface
    m.interior = medium_wax(p);
    m.surf.transmit = 1.0;
    return m;
}

Medium medium_glass(vec3 p){
    Medium m = defaultMedium();
    m.ior    = 1.5;
    m.absorb = absorbFor(vec3(0.85, 0.92, 0.9), 2.0);
    return m;
}
Material material_glass(vec3 p, inout Vector n){
    Material m = defaultMaterial();      //glass
    m.interior = medium_glass(p);
    m.surf.transmit = 1.0;
    return m;
}
...
float trace_core(Vector tv){
    return sphereTrace(tv, CORE_P, CORE_RADIUS);
}

float trace_shell(Vector tv){
    return sphereTrace(tv, SHELL_P, SHELL_RADIUS);
}

float trace_wax(Vector tv){
    return sphereTrace(tv, WAX_P, WAX_RADIUS);
}

float trace_glass(Vector tv){
    return sphereTrace(tv, GLASS_P, GLASS_RADIUS);
}
...
float trace_Scene(Vector tv){
    float d = maxDist;
    d = min(d, trace_core(tv));
    d = min(d, trace_shell(tv));
    d = min(d, trace_wax(tv));
    d = min(d, trace_glass(tv));
