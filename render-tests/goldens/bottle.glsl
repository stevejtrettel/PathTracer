...
const vec3  BOTTLE_P          = vec3(1.0, 0.48, 2.0);
const float BOTTLE_BASERADIUS = 1.25;
const float BOTTLE_BASEHEIGHT = 1.5;
const float BOTTLE_NECKRADIUS = 0.3;
const float BOTTLE_NECKHEIGHT = 1.0;
const float BOTTLE_THICKNESS  = 0.1;
const float BOTTLE_ROUNDED    = 0.1;
const float BOTTLE_SMOOTHJOIN = 0.3;
const float BOTTLE_BUMP       = 1.0;
...
float sdf_bottle(vec3 p){
    return bottleDistance(p - BOTTLE_P, BOTTLE_BASERADIUS, BOTTLE_BASEHEIGHT, BOTTLE_NECKRADIUS, BOTTLE_NECKHEIGHT, BOTTLE_THICKNESS, BOTTLE_ROUNDED, BOTTLE_SMOOTHJOIN, BOTTLE_BUMP);
}
...
bool inside_bottle(vec3 p){ return sdf_bottle(p) < 0.0; }
...
float bound_bottle(vec3 p){
    return bottleBound(p - BOTTLE_P, BOTTLE_BASERADIUS, BOTTLE_BASEHEIGHT, BOTTLE_NECKHEIGHT, BOTTLE_THICKNESS, BOTTLE_ROUNDED, BOTTLE_SMOOTHJOIN);
}
...
Vector normal_bottle(vec3 p){
    vec2 k = vec2(1.0,-1.0)*0.5773;
    return Vector(p, normalize( k.xyy*sdf_bottle(p + k.xyy*NRM_E) + k.yyx*sdf_bottle(p + k.yyx*NRM_E)
                              + k.yxy*sdf_bottle(p + k.yxy*NRM_E) + k.xxx*sdf_bottle(p + k.xxx*NRM_E) ));
}
...
Medium medium_bottle(vec3 p){
    Medium m = defaultMedium();
    m.ior    = 1.5;
    m.absorb = 1.5*vec3(0.25, 0.65, 0.4);
    m.mfp    = 0.5*sssDensity;
    m.blur   = sssScatter;
    return m;
}
Material material_bottle(vec3 p, inout Vector n){
    Material m = defaultMaterial();      //subsurface
    m.interior = medium_bottle(p);
...
Vector normalOf(int id, vec3 p){
    if(id == ID_BOTTLE){ return normal_bottle(p); }
...
float sdf_Scene(Vector tv){
    ...
    float b_bottle = bound_bottle(p);
    d = min(d, (b_bottle > BOUND_MARGIN) ? b_bottle : sdf_bottle(p));
